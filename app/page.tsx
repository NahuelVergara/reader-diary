  ...
  return (
    <main className="ml-0 md:ml-64 flex-1 p-6 md:p-16 w-full">
      <header className="mb-16 md:hidden">
        <h1 className="font-headline-md text-headline-md italic text-text-main">La Biblioteca</h1>
      </header>
      ...
      {sortedYears.map((year) => (
        <div key={year} className="mb-24">
          ...
          <div className="relative flex items-center py-5 mb-12">
            ...
            <span className="flex-shrink-0 mx-4 font-meta-label text-meta-label text-text-main">{year}</span>
            <div className="flex-grow border-t border-muted"></div>
          </div>
            ...
            {groupedEntries[year].map((book, index) => {
              ...
              return (
                  ...
                  <article className="h-full">
                    <div className="aspect-[3/4] bg-surface-card border border-muted p-6 flex flex-col justify-center items-center text-center transition-transform duration-500 ease-out group-hover:-translate-y-2 relative overflow-hidden shadow-[0_10px_30px_-15px_rgba(0,0,0,0.05)] dark:shadow-[0_10px_30px_-15px_rgba(0,0,0,0.4)] h-full">
                      <div className="absolute inset-0 bg-background-light/10 opacity-0 group-hover:opacity-100 transition-opacity duration-300 mix-blend-multiply pointer-events-none dark:mix-blend-screen"></div>
                      ...
                      <div className="w-8 h-px bg-muted mb-4"></div>
                      <p className="font-meta-label text-meta-label text-on-surface-variant uppercase">{book.author}</p>
                    </div>
                  </article>
                </Link>
              );
